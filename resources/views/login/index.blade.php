...
<body class="bg-stone-900 text-stone-300 antialiased">
    ...
    <div class="relative min-h-screen flex items-center justify-center overflow-hidden">
        ...
        <div
            class="absolute top-0 -left-1/4 w-96 h-96 bg-amber-500/20 rounded-full filter blur-3xl opacity-40 animate-pulse">
        </div>
        <div
            class="absolute bottom-0 -right-1/4 w-96 h-96 bg-amber-500/20 rounded-full filter blur-3xl opacity-40 animate-pulse animation-delay-4000">
        </div>

        <div
            class="relative z-10 w-full max-w-md p-8 space-y-6 bg-stone-900/60 backdrop-blur-xl border border-stone-700 rounded-2xl shadow-2xl shadow-amber-500/10">
    ...
    @if (Session::has('error'))
        <div id="toast-error"
            class="fixed bottom-5 right-5 p-4 rounded-lg shadow-lg bg-red-900/80 backdrop-blur-sm border border-red-700 text-red-300 transition-opacity duration-500">
            <span>{{ Session('error') }}</span>
        </div>
        <script>
